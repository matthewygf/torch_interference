...
def count_pad2d(m, x, y):
    m.total_ops = torch.Tensor([int(8)])


def count_mbconv_misc(m, x, y):
    x = x[0]
    if not m.id_skip:
        t = torch.Tensor([int(0)])
    else:
        # drop_connect ( divide , multiply )
        # skip_connect ( add )
        # squeeze excite ( sigmoid,multiply=4)
        # very rough estimate
        t = torch.Tensor([int(y.numel()*3 * 4)])
    m.total_ops = t
