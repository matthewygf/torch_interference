...
class VGG(tf.keras.Model):
  def __init__(self, name, config, batch_norm, num_classes=10, data_format='channels_first', input_shape=None):
    ...
    self.features = make_layers(configs[config], batch_norm=batch_norm, data_format=data_format, input_shape=input_shape)
...
def make_layers(config, batch_norm=False, input_channels=3, data_format='channels_first', input_shape=None):
  ...
  for v in config:
    if v == 'M':
      ...
      zeropad = tf.keras.layers.ZeroPadding2D(data_format=data_format, input_shape=input_shape)
      conv2d = tf.keras.layers.Conv2D(v, 3, data_format=data_format)
      if batch_norm:
        layers.add(zeropad)
        layers.add(conv2d)
